<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="stylesheet" href="css/index.css">
    <link rel="stylesheet" href="css/mobiles.css">


    <title>Cataratas</title>
</head>
<body>
<div class="banner-sect">
    <div class="info-banner">
        <img src="img/Globo.svg" class="globe-img">
        <div class="elementos-banner">
            <a href="http://www.salauno.com.mx" class="logo-salauno"><img src="img/salauno-logo.svg"></a>
            <div class="title-baner">Cirugía de cataratas</div>
            <div class="subtitle-baner">Para ver de nuevo <br>el lado brillante de la vida</div>
            <div class="costo-text">Costo $350 MXN</div>
            <div class="button-sala-uno button-agenda-cita">
                <div class="icon-agenda"><img src="img/icon-libro.svg" alt="icon-libro">Agenda tu cita</div>
            </div>
            <div class="hand-icon"></div>
        </div>
        </div>
    <div class="main-img">
        <img src="img/main-bgr.jpg">
    </div>
</div>
